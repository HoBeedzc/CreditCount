  ...
  render() {
    return (
      <Table.Row positive={this.props.isChecked}>
        ...
        <Table.Cell>
          {this.props.major.split("|").map((major, index) => {
            return <Label key={index}>{major}</Label>;
          })}
        </Table.Cell>
      </Table.Row>
    );
...
class CourseList extends React.Component {
  ...
  handleDropdownChange(info) {
    if (this.state.pageSize !== parseInt(info.value)) {
      ...
      this.setState({
        maxPage:
          this.state.dataLength % parseInt(info.value) === 0
            ? parseInt(this.state.dataLength / parseInt(info.value))
            : parseInt(this.state.dataLength / parseInt(info.value)) + 1,
      });
    }
  }
  ...
  refreshState(data) {
    if (this.state.maxPage * this.state.pageSize < data.length) {
      ...
      this.setState({
        ...
        maxPage:
          data.length % this.state.pageSize === 0
            ? parseInt(data.length / this.state.pageSize)
            : parseInt(data.length / this.state.pageSize) + 1,
      });
    }
  }
  ...
  renderCourseList() {
    ...
    courseInfo = courseInfo.slice(
      (this.state.curPage - 1) * this.state.pageSize,
      this.state.curPage * this.state.pageSize
    );

    return <Table.Body>{courseInfo}</Table.Body>;
  }
